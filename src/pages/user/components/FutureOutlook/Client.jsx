...
// New styled component for the "Visit Website" button
const VisitWebsiteButton = styled.a`
  display: inline-block;
  background: linear-gradient(90deg, #0d6efd, #6c757d); /* Matching the Watch Video gradient */
  border: none;
  color: #fff !important; /* Ensure text color is white */
  font-weight: bold;
  padding: 0.375rem 0.75rem; /* Equivalent to btn-sm padding */
  font-size: 0.875rem; /* Equivalent to btn-sm font-size */
...
const OurClients = ({ isPreview = false }) => {
  ...
  if (error) {
    return (
      <section className="py-5 bg-white" id="clients">
        <Container>
          <h2 className="text-center fw-bold mb-5 text-warning">No Clients Yet!</h2>
        </Container>
      </section>
    );
  }
                  ...
                  {clients.map((client, index) => (
                    <Col md={4} key={client.id || index}>
                      <AnimatedSection index={index + 1}>
                        ...
                        {client.website && (
                          <VisitWebsiteButton // Use the new styled component here
                            href={client.website}
                            target="_blank"
      ...
      <Container>
        ...
        <CustomSection bgLight className="p-5">
          ...
          <p className="text-muted text-center mb-4">
            At Wyenfos Infotech, we prioritize building strong, lasting relationships with our clients through dedication and excellence.
          </p>
          <Row className="g-4">
